...
FROM
    POSTS
WHERE LENGTH(content) >= 30 -- 장문 게시물을 쓴 사용자만 조회
GROUP BY user_id -- 유저 아이디 별로 묶음 (집계)
HAVING COUNT(*) >= 5 -- 집계 필터링
;
...
FROM
    LIKES
WHERE
    creation_date >= TO_DATE('2024-01-01', 'YYYY-MM-DD') -- 1. 개별 '좋아요' 데이터를 먼저 필터링
GROUP BY
    post_id -- 2. 게시물 ID 별로 그룹화
HAVING
    COUNT(*) >= 20 -- 3. 그룹별 '좋아요' 수가 20개 이상인 그룹만 필터링
;
